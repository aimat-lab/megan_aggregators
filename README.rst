...
This repository implements the training of a self-explaining MEGAN graph neural network model for the
...
.. code-block:: shell
    ...
    python3 -m megan_aggregators.cli list

The details for a specific experiment can be viewed like this:

.. code-block:: shell

    python3 -m megan_aggregators.cli info [experiment_name]

A new run of an experiment can be started like this.

.. code-block::

    python3 -m megan_aggregators.cli run [experiment_name]
...
``megan_aggregators/experiments``. The subfolders in that folder contain the archived experiments.
